...
export function getPublicUserDashboardData(): UserDashboardData[] {
  ...
  const articleImages = ['1556761175-5973dc0f32e7', '1551836022-d5d88e9218df', '1552664730-d307ca884978', '1454165804606-c3d57bc86b40', '1521737711867-e3b97375f902', '1556761175-b413da4baf72', '1553877522-43269d4ea984', '1551836022-1bd8b4c9b1b0']
  const blogImages = ['1500534623283-312aade485b7', '1516321318423-f06f85e504b3', '1499750310107-5fef28a66643', '1499750310107-5fef28a66643', '1511988617509-a57c8a288659', '1470252649378-9c29740c9fa8', '1500530855697-b586d89ba3ee', '1504306666337-3a6b1a7c8f7b']
  const eventImages = ['1515169067868-5387ec356754', '1475721027785-f74eccf877e2', '1517457373958-b7bdd4587205', '1540575467063-178a50c2df87', '1511578314322-379afb476865', '1505373877841-8d25f7d46678', '1515187029135-18ee286d815b', '1504052434569-70ad5836ab65']

  return names.map((name, index) => {
    const email = `public-member-${index + 1}@kbn.demo`
    const avatar = `https://i.pravatar.cc/96?img=${index + 10}`
    const articleImage = `https://images.unsplash.com/photo-${articleImages[index % articleImages.length]}?w=900&h=520&fit=crop&auto=format`
    const blogImage = `https://images.unsplash.com/photo-${blogImages[index % blogImages.length]}?w=900&h=520&fit=crop&auto=format`
    const eventImage = `https://images.unsplash.com/photo-${eventImages[index % eventImages.length]}?w=900&h=520&fit=crop&auto=format`
    ...
    const article = { id: `public-article-${index + 1}`, featuredImage: articleImage, title: articleTitles[index], topic, excerpt: articleContent, content: articleContent, author: name, publishedDate: `September ${index + 1}, 2025`, status: 'Published' as const, visibility: 'Public' as const }
    const blog = { id: `public-blog-${index + 1}`, featuredImage: blogImage, title: blogTitles[index], topic, excerpt: blogContent, content: blogContent, author: name, publishedDate: `August ${index + 1}, 2025`, status: 'Published' as const, visibility: 'Public' as const }
    const event = { id: `public-event-${index + 1}`, eventImage, title: eventTitles[index], description: `Join ${name} and fellow members for an encouraging gathering focused on practical faith, connection, and service.`, eventType: topics[index % topics.length], startDate: `October ${index + 1}, 2025 at ${8 + (index % 5)}:00 AM`, endDate: `October ${index + 1}, 2025 at ${10 + (index % 5)}:00 AM`, location: index % 3 === 0 ? 'Online via Zoom' : `${city} Community Center`, online: index % 3 === 0, eventUrl: `kbn.org/events/${eventTitles[index].toLowerCase().replace(/[^a-z]+/g, '-')}`, status: 'Published' as const }
...
export function createUserContent<T extends UserContentItem>(email: string, type: UserContentType, item: T) {
  const current = getUserDashboardData(email, { email, name: '', role: 'user', avatar: '' })
  const stored = JSON.parse(localStorage.getItem(DASHBOARD_DATA_KEY) ?? '{}') as Record<string, Partial<UserDashboardData>>
  stored[email.toLowerCase()] = { ...(stored[email.toLowerCase()] ?? {}), [type]: [...current[type], item] }
  localStorage.setItem(DASHBOARD_DATA_KEY, JSON.stringify(stored))
}
